...
# 3) 在 AUTO-TREE 区块顶部添加“最近构建时间（北京时间）”
...
last_commit_time <- function(path) {
  ...
  if (is.na(ts)) return("")
  format(as.POSIXct(ts, origin = "1970-01-01", tz = "Asia/Shanghai"),
         "%Y-%m-%d %H:%M:%S")
}
...
message("README.md 已按标记区间写入：脚本清单 & 目录结构（含北京时间构建时间）。")
